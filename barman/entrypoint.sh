...
[[ "$DB_STREAMING_USER" != "" ]] || ( echo 'Variable DB_USER is not set!' ;exit 5 )
#[[ "$DB_STREAMING_PASSWORD" != "" ]] || ( echo 'Variable DB_PASSWORD is not set!' ;exit 6 )
...
streaming_conninfo = host=$DB_HOST user=$DB_STREAMING_USER password=$DB_PASSWORD port=$DB_PORT
